...
def process_frames(frame, origin, locked, distance, tracking_vertical=False):
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            others_curled = all(hand_landmarks.landmark[tip].y > hand_landmarks.landmark[pip].y 
                              for tip, pip in zip(finger_tips[1:], finger_pips[1:]))
            index_only_up = index_up and others_curled
            
            # INDEX + PINKY UP: Vertical mode (middle+ring curled)
            middle_ring_curled = all(hand_landmarks.landmark[tip].y > hand_landmarks.landmark[pip].y 
                                   for tip, pip in zip([12, 16], [10, 14]))
            index_pinky_up = index_up and pinky_up and middle_ring_curled
            
            # FIXED: INDEX + MIDDLE UP: Swarm (ring+pinky curled)
            ring_pinky_curled = all(hand_landmarks.landmark[tip].y > hand_landmarks.landmark[pip].y 
                                  for tip, pip in zip([16, 20], [14, 18]))
            index_middle_up = index_up and middle_up and ring_pinky_curled
            
            # DEBUG: Print all gesture states
            ...
            if not origin_locked:
                ...
                # FIXED: Check SWARM FIRST when locked (before reset)
                if index_middle_up:
                    call_swarm = True
            ...
            if origin_locked:
                ...
                ready_texts = []
                if fist_detected: ready_texts.append('FIST READY! (Horizontal)')
                if index_pinky_up: ready_texts.append('INDEX+PINKY READY! (Vertical)')
                if index_middle_up: ready_texts.append('INDEX+MIDDLE = SWARM')
                
                cv2.putText(frame_copy, 'FIST=Horizontal | INDEX+PINKY=Vertical', (10, 30),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
                for i, text in enumerate(ready_texts):
                    cv2.putText(frame_copy, text, (10, 70 + i*25),
    ...
    return (frame_copy, (origin_x, origin_y), origin_locked, distance, 
            current_tracking_vertical, call_swarm)
